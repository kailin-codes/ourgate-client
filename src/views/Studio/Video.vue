            ...
            <v-tabs-items v-model="tab">
                  <v-tab-item>
                        <template>
                              <v-card flat>
                                    <v-card-title>
                                          <v-text-field v-model="search" append-icon="mdi-magnify" label="Filter"
                                                single-line hide-details></v-text-field>
                                    </v-card-title>
                                    <v-data-table no-data-text="No videos available, please upload video."
                                          :headers="headers" :items="videos" :search="search" :loading="loading"
                                          loading-text="Loading... Please wait">
                                          <template v-slot:item.feelings="{ item }">
                                                <span class="mr-3"><v-icon small class="pr-1">mdi-thumb-up</v-icon>{{
                                                      item.likes ? item.likes.length : 0 }}</span>
                                                <span><v-icon small class="pr-1">mdi-thumb-down</v-icon>{{ item.dislikes
                                                      ? item.dislikes.length : 0 }}</span>
                                          </template>
                                          <template v-slot:top>
                                                <v-dialog v-model="dialogDelete" persistent max-width="500px">
                                                      <v-card>
                                                            <v-card-title>
                                                                  <span class="headline">Permanently delete this
                                                                        video?</span>
                                                            </v-card-title>
                                                            ...
                                                            <v-card-actions>
                                                                  ...
                                                                  <v-btn color="blue darken-1" text
                                                                        @click="dialogDelete = !dialogDelete">Cancel</v-btn>

                                                                  <v-btn :loading="deleteBtnLoading"
                                                                        color="blue darken-1" text
                                                                        @click="deleteItem">Delete Forever</v-btn>
                                                            </v-card-actions>
                                                      </v-card>
                                                </v-dialog>
                                          </template>
...
export default {
      data: () => ({
            ...
            headers: [
                  ...
                  { text: "Comments", value: "comments", format: value => value ? value.length : 0 },
      ...
      methods: {
            ...
            async getVideos() {
                  ...
                  try {
                        const response = await VideoService.getAll({ userId: user._id, limit: 0 });
