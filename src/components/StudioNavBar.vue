      ...
      <nav id="studioBar">
        <v-app-bar class="white" app clipped-left>
          ...
          <v-menu offsetY>
              ...
              <v-tooltip bottom>
                <template v-slot:activator="{ on: tooltip }">
                  <v-btn
                    ...
                    v-on="{ ...tooltip, ...menu }"
                  >
                    <v-icon size="25" class="red--text small">mdi-video-plus</v-icon>
                    Create
                  </v-btn>
                </template>
            ...
            <v-list>
              <v-list-item @click="openUploadVideoModal">
          ...
          <v-menu offset-y left>
            <template v-slot:activator="{ on }">
              <v-btn small color="red" depressed fab v-on="on" class="white--text">
                <v-avatar v-if="currentUser && currentUser.photoUrl !== 'no-photo.jpg'">
                  <img
            ...
            <v-card v-if="currentUser">
              ...
              <v-list>
                <v-list-item :to="`/channels/${currentUser._id}`">
                  <v-list-item-icon>
              ...
              <v-list class="text-center">
                <v-list-item class="px-2">
                  <v-btn
                    ...
                    depressed
                    fab
                    :to="currentUser ? `/channels/${currentUser._id}` : ''"
                    class="white--text mx-auto"
                  >
                    <v-avatar
                      height="96"
                      width="96"
                      v-if="currentUser && currentUser.photoUrl !== 'no-photo.jpg'"
                    >
                      <img
                        :src="`${getUrl}/uploads/avatars/${currentUser.photoUrl}`"
                        :alt="`${currentUser.channelName} avatar`"
                    ...
                    <span v-else-if="currentUser" class="display-3">
                      {{ currentUser.channelName.split('')[0].toUpperCase() }}
                    </span>
                  </v-btn>
                </v-list-item>

                <v-list-item v-if="currentUser" :to="`/channels/${currentUser._id}`">
                  <v-list-item-content>
        ...
        <upload-video-modal
          :open-dialog="uploadVideoDialog"
          @closeDialog="closeUploadVideoModal"
        />
        <settings-modal
          :open-dialog="settingsDialog"
          @closeDialog="closeSettingsModal"
        />
      </nav>
    </template>
